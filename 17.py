# The string "dir\n\tsubdir1\n\tsubdir2\n\t\tfile.ext" represents:
# dir
#     subdir1
#     subdir2
#         file.ext
# The directory dir contains an empty sub-directory subdir1 and a sub-directory subdir2 containing a file file.ext.
# The string "dir\n\tsubdir1\n\t\tfile1.ext\n\t\tsubsubdir1\n\tsubdir2\n\t\tsubsubdir2\n\t\t\tfile2.ext" represents:
# dir
#     subdir1
#         file1.ext
#         subsubdir1
#     subdir2
#         subsubdir2
#             file2.ext
# The directory dir contains two sub-directories subdir1 and subdir2. subdir1 contains a file file1.ext and an empty second-level sub-directory subsubdir1.
# subdir2 contains a second-level sub-directory subsubdir2 containing a file file2.ext.
#
# We are interested in finding the longest (number of characters) absolute path to a file within our file system.
# For example, in the second example above, the longest absolute path is "dir/subdir2/subsubdir2/file2.ext", and its length is 32 (not including the double quotes).
# Given a string representing the file system in the above format, return the length of the longest absolute path to a file in the abstracted file system. If there is no file in the system, return 0.
#
# Note:
# The name of a file contains at least a period and an extension.
# The name of a directory or sub-directory will not contain a period.

# \t is tab
# \n is new line

def longest_path(file_system:str)->int:
    file_system = file_system.split('\n')

    current_working_directory = [file_system[0]]
    path_length = 0

    for token in file_system[1:]:
        directory_depth = 0
        while token[directory_depth] == '\t':
            directory_depth += 1

        #Accessing nested directory/file
        if directory_depth == len(current_working_directory):
            #Is file
            if '.' in token:
                path_length = max(path_length,len('/'.join(current_working_directory)+'/'+token.replace('\t','')))
            #Is Directory
            else:
                current_working_directory.append(token.replace('\t',''))
        #Exiting a directory
        else:
            current_working_directory = current_working_directory[:directory_depth]
            current_working_directory.append(token.replace('\t',''))




if __name__ == "__main__":
    fileSystem = "dir\n\tdirsubr1\n\t\tfile1.ext\n\t\tsubsubdir1\n\tsubdir2\n\t\tsubsubdir2\n\t\t\tfile2.ext"
    longest_path(fileSystem)
    #directory = Directory(fileSystem)

